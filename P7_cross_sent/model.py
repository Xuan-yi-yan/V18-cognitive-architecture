...
class P7WordRouter2048(nn.Module):
    ...
    def __init__(self, word_dim=128, inner_dim=128, heads=32, head_dim=4, max_len=32,
        ...
        # 输入投影: P1(128) → 内部维度 (word_dim≠inner_dim时)
    ...
    def forward(self, A_word_vecs, B_word_vecs, last_loss=1.0):
        ...
        q = self.q_proj(A_with_pos)  # [nA, 128]
        k = self.k_proj(B_word_vecs) # [nB, 128]
        hd = self.head_dim
        ...
        return word_out, sent_vec, attn, word_gate, sent_gate  # 返回gate用于sparsity loss
